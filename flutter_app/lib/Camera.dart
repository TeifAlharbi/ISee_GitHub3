...
class _Camera extends State<Camera> with SingleTickerProviderStateMixin {
    ...
    return Scaffold(
      body: Stack(
        children: <Widget>[
                    ...
                    children:
                      ...
                      if (document.id == currentUser.uid) {
                        ...
                        Future.delayed(Duration(), () async {
                          await setState(() {
                            print(
                                "-------------1 " + document.data()["CVDType"]);
                            cvd = document.data()["CVDType"];
                          });
                        });
                        return new ListTile();
                      } else {
          ...
          ), //----------Get Color-blindness type of the user from firebase----------
        ],
      ),
    );
  }
        ...
        child: outputs != null && outputs.isNotEmpty
            ? ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  //Frame the object & Label the Color name
  ...
  //Label the Color name & Frame the object (Based on the Color-blindness type)
  displayColor(int index) {
    //1-Check the user color-blindness type
    if (cvd == "Duetronopia" || cvd == "Protanopia") {
      print("------------- person is Duetronopia or  Protanopia");
      //Blue&Yellow are seen colors to this type
      //So, any other color is unseen and should be displayed
      if (outputs[index].label != "Blue" && outputs[index].label != "Yellow") {
        print("------------- color is not blue or yellow");
        return Container(
          height: 300,
          //1- Frame the object
          decoration: BoxDecoration(
          ...
          //2- Label the Color name
          child: Text(
            "${outputs[index].label}  ${(outputs[index].confidence * 100.0).toStringAsFixed(2)} %",
      ...
      print("------------- person is Tritanopia");
      //All the colors are unseen or confused to this type
      //So, all the colors should be displayed
      return Container(
        height: 300,
        //1- Frame the object
        decoration: BoxDecoration(
